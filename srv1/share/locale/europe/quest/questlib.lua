...
table_contains = function(table_ex, value_ex)
	for _, value in ipairs(table_ex) do
		if value == value_ex then
			return true;
		end
    end
...
function CalculateTime(second)
    ...
    local days = math.floor(second/86400)
    local hours = math.floor(math.mod(second, 86400)/3600)
    local minutes = math.floor(math.mod(second,3600)/60)
    local seconds = math.floor(math.mod(second,60))
    local timeText = ""
    if days > 0 then
        timeText = ""..days.."d "
    end
