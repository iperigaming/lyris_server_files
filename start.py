		...
		for dic1 in tmpProcList:
			...
			if v_system in ("FreeBSD", "Linux"):
				if not fShell("""ps afx | fgrep "./%s" | fgrep -v grep | awk '{print $1}'"""%dic1["name"], True):
					...
					# goto process file
					print("-> starting \033[92m%s\x1b[0m" % dic1["name"])
					os_chdir(dic1["path"])
					# start process
					# print("./%s &"%dic1["name"])
					# fShell("./%s &"%dic1["name"])
					fShell("./%s %s >> %s.log 2>&1 &" % (dic1["name"], addFlags, dic1["name"]))
					# fShell("./vrunner --daemon --pid-path=pid.core --file=%s >> core_log.log 2>&1 &" % (dic1["name"]))
					# cd db && ./vrunner --daemon --pid-path=pid.db --file=db >> db.log 2>&1 &
					# goto base again
					os_chdir(szPWD)
					# time.sleep(1)
